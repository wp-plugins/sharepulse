	...
	public static function get_counts($id) {
		...
		foreach ($services as $name => $service) {
			...
			if ( is_wp_error( $result ) ) {
				...
				error_log("Please submit the following error to http://www.jackreichert.com/contact/ -- $error_string");
			} else {
...
<?php	
	...
	<?php endforeach;
		...
		if ( is_singular( 'post' ) ) {
			global $post;
			if ( intval( get_post_meta( $post->ID, 'sp_last_updated', true ) ) < ( time() - 60 ) ) {
				update_post_meta( $post->ID, 'sp_last_updated', time() );
